...
export class RatingReportComponent {
  ...
  displayedColumns: string[] = ['sr', 'desc', 'restaurant','reportedTo', 'reported', 'score', 'reported_date', 'status', 'action'];
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/report/list?type=3&page=${this.currentPage + 1}&page_size=${this.pageSize}&is_paginated=true${query}`).subscribe((res: any) => {
