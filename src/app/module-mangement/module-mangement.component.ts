...
@Component({
  selector: 'app-module-mangement',
  templateUrl: './module-mangement.component.html',
  styleUrls: ['./module-mangement.component.scss']
})
export class ModuleMangementComponent {
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/module/get?page=${ this.currentPage+1}&page_size=${ this.pageSize}&is_paginated=true&sort_by=created_at&order=desc`).subscribe((res: any) => {
  ...
  addUpdateModule(moduleData: any = {}) {
  ...
  removeData(element:any){
    ...
    this._appService.deleteApi(`/admin/module/delete`, sendBody, 1).subscribe({
      next: (success: any) => {
        if (success.success) {
