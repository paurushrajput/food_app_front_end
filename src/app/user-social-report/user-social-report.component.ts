...
@Component({
  selector: 'app-user-social-report',
  templateUrl: './user-social-report.component.html',
  styleUrls: ['./user-social-report.component.scss']
})
export class UserSocialReportComponent {
  ...
  displayedColumns: string[] = ['sr', 'desc', 'reportedTo', 'reported', 'reported_date', 'status','action'];
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/report/list?type=1&page=${this.currentPage + 1}&page_size=${this.pageSize}&is_paginated=true${query}`).subscribe((res: any) => {
