...
export class PermissionComponent {
  ...
  displayedColumns: string[] = ['sr','name', 'type','status', 'created','action'];
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/permission/get?page=${ this.currentPage+1}&page_size=${ this.pageSize}&is_paginated=true&sort_by=created_at&order=desc`).subscribe((res: any) => {
  ...
  addUpdatePermission(permissionData: any = {}) {
  ...
  removeData(element:any){
    ...
    this._appService.deleteApi(`/admin/permission/delete`, sendBody, 1).subscribe({
      next: (success: any) => {
        if (success.success) {
