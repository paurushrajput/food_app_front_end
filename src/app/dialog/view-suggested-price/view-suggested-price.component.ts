...
@Component({
  selector: 'app-view-suggested-price',
  templateUrl: './view-suggested-price.component.html',
  styleUrls: ['./view-suggested-price.component.scss']
})
export class ViewSuggestedPriceComponent {
  ...
  displayedColumns: string[] = ['sr', 'createdAt', 'userName', 'email','title','level'];
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/social/suggested-price-list?res_id=${this.data?.postData?.restaurant_uid}&page=${this.currentPage + 1}&page_size=${this.pageSize}`).subscribe((res: any) => {
