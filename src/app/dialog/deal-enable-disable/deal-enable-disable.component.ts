...
@Component({
  selector: 'app-deal-enable-disable',
  templateUrl: './deal-enable-disable.component.html',
  styleUrls: ['./deal-enable-disable.component.scss']
})
export class DealEnableDisableComponent {
  ...
  ngOnInit(): void {
    ...
    this.text = this.selectedDeal.status == 1 ? 'Disable' : 'Enable'
    if (this.data)
      this.setFormvalues()
  }
  ...
  updateDeal() {
    let data = {
      ...
      status: Number(this.selectedDeal.status == 1 ? 0 : 1)
    }
