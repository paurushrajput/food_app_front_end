...
@Component({
  selector: 'app-view-suggested-cuisines',
  templateUrl: './view-suggested-cuisines.component.html',
  styleUrls: ['./view-suggested-cuisines.component.scss']
})
export class ViewSuggestedCuisinesComponent {
  ...
  displayedColumns: string[] = ['sr', 'createdAt', 'userName', 'email','category'];
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/social/user-suggested-cusines-list?res_id=${this.data?.postData?.restaurant_uid}&page=${this.currentPage + 1}&page_size=${this.pageSize}`).subscribe((res: any) => {
