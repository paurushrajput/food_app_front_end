...
@Component({
  selector: 'app-waste-to-earn-management',
  templateUrl: './waste-to-earn-management.component.html',
  styleUrls: ['./waste-to-earn-management.component.scss']
})
export class WasteToEarnManagementComponent {
  payload: any
  uid: any
  imageSrc: any
  ...
  displayedColumns: string[] = ['sr', 'title', 'icon', 'coins', 'cn', 'button_name', 'status', 'action'];
  ...
  edit(data: any = {}) {
    const dialogRef = this.dialog.open(EditWasteToEarnComponent, {
      hasBackdrop: false,
      // width: '80px',
      // height: '80px',
      width: '575px',
  ...
  private updateBackendWithNewOrder(data: any): void {
    ...
    this._appService.putApiWithAuth(`/admin/ways-to-earn/update-sequence`, sendBody, 1)
      .subscribe(
        response => {
          this._appService.success('Sequence updated successfully')
          this.loadData()
        },
        error => {
  ...
  onToggleChange(event: any, element: any, index: any) {
    this.isChecked = element.status == 1 ? true : false
    if (event.checked) {
      this.updateToggleChange(1, element.id)
      // this.payload[index].status = 1;
    } else {
      // this.payload[index].status = 0;
      this.updateToggleChange(0, element.id)
    }
    // this.updateToggleChange(this.payload)
  }
  updateToggleChange(status: any, id: any) {
    let payload = {
      "id": id,
      "status": status
    }
    this._appService.putApiWithAuth(`/admin/ways-to-earn/update`, payload, 1).subscribe({
      next: (success: any) => {
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/ways-to-earn/list`).subscribe((res: any) => {
