...
import { CusinePopupComponent } from 'src/app/cusine-popup/cusine-popup.component';
...
@Component({
  selector: 'app-cusines-list',
  templateUrl: './cusines-list.component.html',
  styleUrls: ['./cusines-list.component.scss']
})
export class CusinesListComponent {
ELEMENT_DATA: CATEGORY[] = [];
  ...
  displayedColumns: string[] = ['sr', 'restaurant_name','phone','address','price_level','createdAt', 'userName', 'email','category','restaurant_cuisines','details'];
  ...
  loadData() {
    ...
    this._appService.getApiWithAuth(`/admin/social/get-all-cusines-suggested?keyword=${this.searchForm.value.name}&status=${this.searchForm.value.status}&page=${this.currentPage + 1}&page_size=${this.pageSize}`).subscribe((res: any) => {
